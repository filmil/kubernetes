...
// errorDialerFunc always returns an error when it dials out.
func errorDialerFunc(network, address string) (net.Conn, error) {
	return nil, fmt.Errorf("no dial for you!")
}
...
func TestAuthorizationWebhookDialer(t *testing.T) {
	tt := []struct {
		...
		// If set, we use a dialer that always errors out.
		useErrorDialer bool
	}{
		{
		...
		},
		{
			...
			expectedName:      "servicename",
			serverAddress:     "http://servicename.servicenamespace.svc",
			// Verify that nil transport is handled correctly.
		...
		},
		{
			...
			useErrorDialer: true,
			expectedError:  fmt.Errorf("no dial for you!"),
		},
		{
			expectedNamespace: "servicenamespace",
			expectedName:      "servicename",
	...
	for i, ttt := range tt {
		...
		if ttt.useErrorDialer {
			defaultTransport.Dial = errorDialerFunc
		}
